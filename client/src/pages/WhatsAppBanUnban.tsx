          ...
          {services.map((service) => {
            ...
            return (
              <article key={service.name} className={`fluxy-card fluxy-card-hover relative flex min-h-[500px] flex-col rounded-2xl p-6 sm:p-7 ${service.cardClass}`}>
                ...
                <div className="mt-5 flex items-end gap-2"><p className="text-4xl font-extrabold leading-none tracking-[-0.06em] text-white">{service.priceDisplay}</p><span className="mb-0.5 text-sm text-[#8f80a7]">/ service</span></div>
                ...
                <button type="button" onClick={() => setSelectedService(service)} className={`mt-auto flex w-full items-center justify-center rounded-xl px-4 py-3.5 text-sm font-extrabold text-white transition hover:brightness-110 ${service.buttonClass}`}>BUY NOW — {service.priceDisplay}</button>
              </article>
            );
          })}
        </section>
